...
export default function CadastroEscritorio1({ api }) {
    ...
    async function handleCadastro(e) {
        ...
        if (!nomeFantasia.trim()) {
            camposFaltando.push('Nome fantasia');
        }
        ...
        if (!oab.trim()) {
            camposFaltando.push('Registro na OAB');
        }

        if (!ufOab.trim()) {
            camposFaltando.push('UF da OAB');
        }
        ...
        const cnpjNumeros = cnpj.replace(
            /\D/g,
            ''
        );
        ...
        const telefoneNumeros = telefone.replace(
            /\D/g,
            ''
        );
        ...
        if (
            ...
            mostrarErro(
                'Telefone incompleto. Digite DDD + número (10 ou 11 dígitos).'
            );
        ...
        try {
            ...
            if (!token) {
                mostrarErro(
                    'Você precisa estar logado para cadastrar um escritório.'
                );
            ...
            if (resposta.ok) {
                ...
                throw new Error(
                    dados.error ||
                    'Não foi possível cadastrar o escritório.'
                );
            }

        } catch (erro) {
            console.error(
                'Erro ao cadastrar escritório:',
                erro
            );

            mostrarErro(
                erro.message ||
                'Erro ao cadastrar escritório. Tente novamente.'
            );
        }
    }
                ...
                >
                    ...
                    >
                        ...
                        {avisoInternetLenta && (
                            ...
                            >
                                Sua internet pode estar lenta ou a OAB
                                está demorando para responder.
