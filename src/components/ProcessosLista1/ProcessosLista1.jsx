...
export default function ProcessosLista1({ api }) {
    ...
    function nomesClientes(processo) {
        if (Array.isArray(processo.clientes)) {
            return processo.clientes.map(c => c.nome || c).join(', ');
        }
    ...
    function handleSalvarEdicao() {
        ...
        setEditando(false);
        mostrarMensagem('Informações atualizadas com sucesso!', 'sucesso');
    }
    ...
    function handleInativar() {
        ...
        fecharModalInativar();
        mostrarMensagem('Processo inativado com sucesso!', 'sucesso');
    }

    function handleAtivar(processo) {
        setProcessos(prev =>
            prev.map(p =>
                p.id === processo.id
        ...
        mostrarMensagem('Processo ativado com sucesso!', 'sucesso');
    }
    ...
    const processosFiltrados = processos.filter(processo => {
        const numeroMatch = (processo.numero || '')
            .toLowerCase()
        ...
        let tipoMatch = true;
        if (filtroTipo !== 'todos') {
            const area = normalizarTexto(processo.tipo || processo.area);
            const filtro = normalizarTexto(filtroTipo);
            if (filtro === 'civil') {
                tipoMatch = area === 'civil' || area === 'civel';
            } else {
                tipoMatch = area === filtro;
            }
        }
                                    ...
                                    <tr key={processo.id}>
                                        <td>
                                            <div className={css.colunaProcesso}>
                                                <svg className={css.iconeProcesso} width="22" height="18" viewBox="0 0 24 20" fill="none" stroke="#0047ab" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                                        ...
                                        <td className={css.colunaAcoes}>
                                            <button className={css.botaoVer} onClick={() => abrirModal(processo)}>Ver</button>
                                            {processo.status === 'inativo' ? (
                                                <button className={css.botaoAtivar} onClick={() => handleAtivar(processo)}>Ativar</button>
                                            ) : (
                                                <button className={css.botaoInativar} onClick={() => abrirModalInativar(processo)}>Inativar</button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
                ...
                <div className={css.modalOverlay} onClick={(e) => { if (e.target === e.currentTarget) fecharModal(); }}>
                    <div className={css.modalContainer}>
                        <div className={css.modalHeader}>
                            <h2 className={css.modalTitulo}>Processo {processoSelecionado.numero}</h2>
                        ...
                        <div className={css.modalBody}>
                            ...
                            <form className={css.formulario} onSubmit={(e) => e.preventDefault()}>
                                <div className={css.secaoTitulo}>
                                    <h3 className={css.secaoSubtitulo}>Dados do Processo</h3>
                                </div>

                                <div className={css.linha}>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Nº do processo</label>
                                        <input type="text" className={css.input} value={processoSelecionado.numero || '--'} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Cliente(s)</label>
                                        <input type="text" className={css.input} value={nomesClientes(processoSelecionado)} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Tipo do processo</label>
                                        <input type="text" className={css.input} value={processoSelecionado.tipo_processo || '--'} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Assunto</label>
                                        {editando ? (
                                            <input type="text" name="assunto" className={css.input} value={dadosEditados.assunto || ''} onChange={handleEditChange} maxLength={254} />
                                        ) : (
                                            <input type="text" className={css.input} value={processoSelecionado.assunto || '--'} readOnly />
                                        )}
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Área</label>
                                        {editando ? (
                                            <input type="text" name="area" className={css.input} value={dadosEditados.area || dadosEditados.tipo || ''} onChange={handleEditChange} maxLength={50} />
                                        ) : (
                                            <input type="text" className={css.input} value={processoSelecionado.area || processoSelecionado.tipo || '--'} readOnly />
                                        )}
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Comarca</label>
                                        <input type="text" className={css.input} value={processoSelecionado.comarca || '--'} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Vara</label>
                                        <input type="text" className={css.input} value={processoSelecionado.vara || '--'} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Instância</label>
                                        <input type="text" className={css.input} value={processoSelecionado.instancia ? `${processoSelecionado.instancia}ª instância` : '--'} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Advogado responsável</label>
                                        <input type="text" className={css.input} value={processoSelecionado.advogado_responsavel || '--'} readOnly />
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Data de início</label>
                                        {editando ? (
                                            <input type="text" name="data_inicio" className={css.input} value={dadosEditados.data_inicio || ''} onChange={handleEditChange} maxLength={10} />
                                        ) : (
                                            <input type="text" className={css.input} value={processoSelecionado.data_inicio || '--'} readOnly />
                                        )}
                                    </div>
                                    <div className={css.campoMetade}>
                                        <label className={css.label}>Status</label>
                                        <input type="text" className={css.input} value={formatarStatus(processoSelecionado.status)} readOnly />
                                    </div>
                                    <div className={css.campoInteiro}>
                                        <label className={css.label}>Descrição</label>
                                        {editando ? (
                                            <textarea name="descricao" className={css.textarea} value={dadosEditados.descricao || ''} onChange={handleEditChange} maxLength={1000} />
                                        ) : (
                                            <textarea className={css.textarea} value={processoSelecionado.descricao || '--'} readOnly />
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className={css.botaoContainer}>
                                    {editando ? (
                                        <>
                                            <button className={css.botaoCadastro} type="button" onClick={handleSalvarEdicao}>Atualizar Informações</button>
            ...
            {modalInativarAberto && processoInativar && (
                <div className={css.modalOverlay} onClick={(e) => { if (e.target === e.currentTarget) fecharModalInativar(); }}>
                    <div className={css.modalInativacao}>
                        ...
                        <p className={css.subtituloInativacao}>Confirme para inativar o processo.</p>
                        <div className={css.botoesInativacao}>
                            <button className={css.btnCancelarInativacao} onClick={fecharModalInativar}>Cancelar</button>
                            <button className={css.btnConfirmarInativacao} onClick={handleInativar}>Inativar</button>
                        </div>
                    </div>
                </div>
            )}
