...
export default function CadastroParteContrariaFisica1({ api }) {
    ...
    function handleNome(e) {
        ...
        if (valor.length <= 254) {
            setNomeCompleto(capitalizarNome(valor));
        }
    }
    ...
    function handleDataNascimento(e) {
        ...
        if (valor.length <= 2) {
            ...
            setDataNascimento(`${valor.slice(0, 2)}/${valor.slice(2)}`);
        } else {
    ...
    function handleCpf(e) {
        ...
        if (valor.length <= 3) {
            ...
            setCpf(`${valor.slice(0, 3)}.${valor.slice(3)}`);
        } else if (valor.length <= 9) {
            setCpf(`${valor.slice(0, 3)}.${valor.slice(3, 6)}.${valor.slice(6)}`);
        } else {
            setCpf(`${valor.slice(0, 3)}.${valor.slice(3, 6)}.${valor.slice(6, 9)}-${valor.slice(9, 11)}`);
        }
    }
    ...
    function handleRg(e) {
        ...
        if (valor.length <= 2) {
            ...
            setRg(`${valor.slice(0, 2)}.${valor.slice(2)}`);
        } else if (valor.length <= 8) {
            setRg(`${valor.slice(0, 2)}.${valor.slice(2, 5)}.${valor.slice(5)}`);
        } else {
    ...
    function handleProfissao(e) {
        const valor = e.target.value.replace(/[^a-zA-ZÀ-ÿ\s]/g, '');
        if (valor.length <= 254) {
            setProfissao(capitalizarNome(valor));
        }
    }
    ...
    function trocarTipoPessoa(novoFisico) {
        ...
        if (!novoFisico) {
            ...
            navigate('/cadastro_parte_contraria_juridica', {
                state: { processo }
            });
        }
    }
    ...
    return (
        <div className={css.paginaCompleta}>
            ...
            <section className={css.containerSection} ref={topoRef}>
                ...
                {mensagem && (
                    <div
                        className={`${css.mensagemContainer} ${tipoMensagem === 'sucesso' ? css.sucesso : css.erro}`}
                    >
                        {mensagem}
                    </div>
                )}
